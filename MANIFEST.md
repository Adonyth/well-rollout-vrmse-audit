...
| The package's own self-descriptions (`[selfclaims]` stage) | `scripts/check_self_claims.py` | Written after reviewers repeatedly found discrepancies by hand in this package's prose, docstrings and packaged producer strings | Yes — packaged, and it gates the exit code. It checks paths, pinned counts, stage order and conditionality, advertised CLI invocations, worked numeric examples, the row-file I/O contract, withdrawal markers, producer targets and the dependency floor. `--validate-guard` reintroduces twenty-four real past discrepancies and requires each to be caught BY THE RULE NAMED FOR IT — nineteen of them in a standalone clone, where five are reported as skipped rather than passed because their subject (e.g. `../RT-QUANT.md`) or their owning rule (`check_instrument_copies`, which compares against a sibling copy) lies outside the package. It is **one-directional**: it checks that what the package says is true, not that everything it ships is described. |
...
# TFNO: the only same-window comparison against an uncensored published cell (6-12, published
# 6.72). FULL test split -- all ten trajectories -- but a TWELVE-step rollout, because
# window 13-30 is censored and autoregressive rollout is prefix-deterministic, so steps
# 1-12 are identical to a 30-step run's. --device cpu: TFNO's 302M parameters exhaust this
# machine's MPS allocator at the 128^3 grid, as FNO's did.
python3 rt_model_eval.py --model TFNO --device cpu \
    --revision 5cccb8597ed98189b63493b3d303cd0d24622b2f \
    --pairs "0:0;0:1;1:0;1:1;2:0;2:1;3:0;3:1;4:0;4:1" \
    --rollout-steps 12 --onestep-starts 0 \
    --output-dir ../fixtures/models --cache-dir ../hf-cache

# Cross-check of that forward path against the library's own dataloader, normalizer and
# VRMSE (HuggingFace bytes vs our SDSC-mirror reads). Needs the Tier-2 stack; the packaged
# log fixtures/models/tfno_rt_crossvalidation.log is what the paper cites.
N_PROBE=3 python3 validate_rt_tfno.py
