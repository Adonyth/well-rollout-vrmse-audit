...
packaged in `fixtures/` (929,537 bytes in total, of which the RT per-frame/per-window scalars are 240,823; the rest is the Rayleigh-Benard checkpoint rows, the benchmark-wide census, and the Gate-2/Gate-3/provenance fixtures). `verify.py` re-derives the 142 enumerated value checks
